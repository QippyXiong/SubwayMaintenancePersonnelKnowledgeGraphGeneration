...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            print(f"【validation】{epoch}/{self.epochs} loss:{avg_val_loss}")
    ...
    def test(self):
        ...
        # with open("./data/preds.txt", "w", encoding="utf-8") as fp:
        #     for item in preds:
        #         fp.write(str(item) + "\n")
...
def main(data_name, model_name):
    ...
    train_file_name = os.path.join(args.data_path, "train.txt")
    validation_file_name = os.path.join(args.data_path, "validation.txt")
    test_file_name = os.path.join(args.data_path, "test.txt")

    train_dataset = NerDataset(train_file_name, args, tokenizer)
    dev_dataset = NerDataset(validation_file_name, args, tokenizer)
    test_dataset = NerDataset(test_file_name, args, tokenizer)
    ...
    test_loader = DataLoader(test_dataset, shuffle=False, batch_size=args.test_batch_size, num_workers=0)
